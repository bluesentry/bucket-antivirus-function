...
#Avoid installing version in base repo
RUN echo 'exclude=*103.6*' >> /etc/yum.conf
...
RUN cp /tmp/usr/bin/clamscan /tmp/usr/bin/freshclam /tmp/usr/lib64/* /usr/lib64/libp11-kit.so.0 /usr/lib64/libpcre.so.1 /opt/app/bin/
